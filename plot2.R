...
# assigning the data of interest to 'data' after subsetting from the NEI dataframe .
data<-aggregate(subset(NEI, fips=="24510")$Emissions, by=list(year = subset(NEI, fips=="24510")$year), FUN=sum)   

# declaring the size of the png graph image that results from the plotting.
png(filename = "plot2.png", width = 480, height = 480) 

# introducing a penalty for displaying scientific notation of integers in the graph and making the graph more readable
options(scipen=2) 

# setting the color, defining x -axis and y-axis, labelling the respective axes, setting the range of the y axis..  
plot <- barplot( data$x, main = "Emissions from PM2.5 for each year in Baltimore.MD(1999-2008)",ylim = c(0,3500), ylab="Emissions from PM2.5(in tons)",xlab="Years",
